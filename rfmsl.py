...
# ------------------------------------------------------------------------------------
#
# Rectangular waveguide port: refactor me to work on rectangular waveguide
#  
# Example start / stop printout:
#
#     WG PORTS      :   X      Y     Z
#     START port #1: [-15.25    -2.4762   0.    ]
#     STOP  port #1: [-15.       2.4762   2.3616]
#
# Rectangular waveguide is generally a 1:2 ratio between H and W 
# ------------------------------------------------------------------------------------
...
def add_ports(FDTD, mesh, models, n):
    ...
    for name in sorted(ports, key=get_portnum):
        ...
        ## Start & Stop values ar e xyz format describing two faces of stl model bbox.
        ## Values are referenced to model origin e.g. Fusion360 origin.

        print(f"\nRWG PORTS      :   X      Y     Z")
        print(f"START port #{port_nr}: {start}")
        print(f"STOP  port #{port_nr}: {stop}")
        print(f"Port Direction: {p_dir}\n")

        # Waveguide port 2D dimension calculation
        # A: Width
        # B: Height
        # Needs better algorithm after basics working
        #
        # Note: Even though port is not actual WG sim seems to crash w/ freq below
        #        cutoff freq.
        #

        a_dim = (abs(start[1]) + abs(stop[1])) / 1000
        b_dim = (abs(start[2]) + abs(stop[2])) / 1000

        print(f"\nWaveguide port A/B dimensions:")
        print(f"A Dimension: {a_dim} meters | B Dimension: {b_dim} meters\n")

        p = FDTD.AddRectWaveGuidePort(port_nr=port_nr, start=start, stop=stop, 
                                      a=a_dim, b=b_dim,      
                                      p_dir=p_dir, mode_name='TE10', excite=excite,  # Propagation direction
...
def smooth_mesh(mesh):
    ...
    # 
    # Default meshing function is super basic. 
    #
    # Needs a better meshing algorithm once shit works.
    #
    #mesh.SmoothMeshLines('all', ((C0 / args.frequency) / STL_UNIT ) / 50)
