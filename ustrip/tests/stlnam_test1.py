...
GOOD_FILE_NAME = ['ustrip', 'mat=silver', 'pri=1', 'n=2']
...
class TestStlNameParserBasic(unittest.TestCase):
    ...
    def test_split_name(self):
        t = ' '.join(GOOD_FILE_NAME)
        fn = self.uut._split_name(t)
        self.assertEqual(len(GOOD_FILE_NAME), len(fn))

        t = '_'.join(GOOD_FILE_NAME)
        fn = self.uut._split_name(t)
        self.assertEqual(len(GOOD_FILE_NAME), len(fn))

    def test_return_args(self):
        r = self.uut._return_args(GOOD_FILE_NAME)
        for i in range(1, len(GOOD_FILE_NAME)-1):
            self.assertEqual(r[i-1], GOOD_FILE_NAME[i])

    def test_get_argval(self):
        a, v = self.uut._get_argval(GOOD_FILE_NAME[1])
    ...
    def test_parse_goodname(self):
        r = self.uut.parse_filename(' '.join(GOOD_FILE_NAME))
        self.assertTrue(r)
        self.assertEqual(len(self.uut.parsed), len(GOOD_FILE_NAME))
