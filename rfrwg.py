...
def add_ports(FDTD, mesh, models, n):
    ...
    for name in sorted(ports, key=get_portnum):
        ...
        ## Start & Stop values are xyz format describing two faces of stl model bbox.
        ...
        ## Get port size
        a_dim = (abs(start[1]) + abs(stop[1]))
        b_dim = (abs(start[2]) + abs(stop[2]))

        p = FDTD.AddRectWaveGuidePort(port_nr=port_nr, start=start, stop=stop, 
                                      a=a_dim, b=b_dim,      # Port dimensions
